...
while running:
    ...
    # User Input
    user_bill = itools.get_float("Enter the Bill Amount\n")
    user_tip_amt = itools.get_float("Enter Tip % Amount\n")
    user_split_amt = itools.get_int("Enter individuals amount to split the bill?\n")
    # Calculations
    final_tip = user_bill * user_tip_amt / 100
    final_bill = user_bill + final_tip
    final_p_person = final_bill / user_split_amt
    credits_p_person = final_bill * 1.75 // user_split_amt
    ...
    user_confirmation = itools.get_menu_selection("Confirm? \nEnter 1 to Confirm, 2 to Decline\n", 1,2)
